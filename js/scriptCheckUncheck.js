...
function displayItems(itemsJson) {
  ...
  for (let i of itemsJson) {
    ...
    html +=
      "<input class='form-check-input border opacity-100' id='checkbox_" +
      i.id +
      "' type='checkbox' name='checked' value='1' " +
      (i.checked ? "checked" : "") +
      " aria-label='Checkbox for following text input' disabled>";
    html += "</button>";
    html += "</div>";
    html +=
      "<input type='text' class='form-control bg-secondary text-white bg-opacity-25 border-0 " +
      (i.checked ? "text-decoration-line-through" : "") +
      "' value='" +
      i.content +
